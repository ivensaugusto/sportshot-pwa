import { Client, Databases, Query } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://apw.simplemsg.net.br/v1')
    .setProject('sportshot')
    .setKey('standard_');

const databases = new Databases(client);

async function checkDocs() {
    try {
        const response = await databases.listDocuments('sportshot-db', 'notices');
        console.log("Total Notices:", response.total);
        response.documents.forEach(doc => {
            console.log(`- [${doc.$id}] Title: ${doc.title} | Sender: ${doc.sender} | CreatedAt: ${doc.$createdAt} | Permissions: ${JSON.stringify(doc.$permissions)}`);
        });
    } catch (err) {
        console.error(err);
    }
}

checkDocs();
